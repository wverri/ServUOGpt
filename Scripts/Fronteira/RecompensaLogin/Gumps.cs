...
namespace Server.Ziden.RecompensaLogin
{
    ...
    public class GuildLoginRewardsGump : BaseRewardGump
    {
        public GuildLoginRewardsGump(Mobile owner, PlayerMobile user)
            : base(owner, user, RecompensasLoginGuilda.Rewards, "Recompensas de Atividade em Guilda")
        {
        }

        public override double GetPoints(Mobile m)
        {
            return PointsSystem.LoginGuilda.GetPoints(m);
        }
        ...
        public override void OnConfirmed(CollectionItem citem, int index)
        {
            ...
            if (item != null)
            {
                ...
                {
                    PointsSystem.LoginGuilda.DeductPoints(User, citem.Points);
